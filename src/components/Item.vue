<template>
  <transition name="slide-fade">
    <div class="list-item  editingClass editing " :class="{checked: item.checked}" v-show="!item.isDelete">
      ...
      <input type="text" v-model="item.record" placeholder='写点什么。。。'  :disabled=" item.checked || locked" @keyup.enter="onChange">
...
export default {
  ...
  methods: {
    ...
    onChange() {
      editRecord({
        id: this.id, record: this.item.record, index: this.index,checked:this.item.checked
      }).then(data => {
        this.init();
